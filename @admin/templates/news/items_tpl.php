<script language="javascript" type="text/javascript">
	...
	$("#xoahet").click(function(){
		...
		if (hoi==true) document.location = "index.php?com=news&type=<?=$_REQUEST['type']?>&act=delete&listid=" + listid;
	});
	});
	...
	{	
		...
		window.location ="index.php?com=news&act=man&type=<?=$_REQUEST['type']?>&id_item="+a.value;	
		return true;
	}
...
<?php
	...
	{
		$sql_huyen="select * from table_news_item where hienthi=1 and type='".$_REQUEST['type']."' order by stt,id desc ";
		$result=mysql_query($sql_huyen);
		$str='
			<select id="id_item" name="id_item" onchange="select_onchange()" class="main_select">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    ...
    <tbody>
       ...
       <td>
        ...
        <td class="actBtns">
            <a href="index.php?com=news&act=edit&id=<?=$items[$i]['id']?>&type=<?=$_REQUEST['type']?><?php if($items[$i]['id_item']!='') echo'&id_item='.$items[$i]['id_item'];?>&hienthi=<?=$items[$i]['hienthi']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" title="" class="smallButton tipS" original-title="Sửa bài viết"><img src="./images/icons/dark/pencil.png" alt=""></a>

            <a href="index.php?com=news&act=delete&id=<?=$items[$i]['id']?>&type=<?=$_REQUEST['type']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" onClick="if(!confirm('Xác nhận xóa <?=$items[$i]['ten']?>')) return false;" title="" class="smallButton tipS" original-title="Xóa bài viết"><img src="./images/icons/dark/close.png" alt=""></a>
        </td>
      </tr>
         <?php } ?>
                </tbody>

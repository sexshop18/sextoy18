...
function show_image ($mime_type, $image_resized, $quality, $cache_dir) {
	...
	$watermark = imagecreatefromfile('upload/sanpham/'.$logo_d['sanpham']);
 	if (!$watermark) die('Unable to open watermark');

// Load

imagecopyresampled($image_resized, $watermark, 0, 0, 0, 0, imagesx($image_resized), imagesy($image_resized), imagesx($watermark), imagesy($watermark));
